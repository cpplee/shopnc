...
$html_title = 'ShopNC本地生活v2.1 - 安装向导';

$html_header = <<<EOF
<div class="header">
  <div class="layout">
    <div class="title">
      <h5>ShopNC本地生活v2.1</h5>
      <h2>系统安装向导</h2>
    </div>
    <div class="version">版本: 2014.06.27.285</div>
  </div>
</div>
...
<div class="footer">
  ...
  <h6>&copy; 2007-2014 <a href="http://www.shopnc.net" title="天津市网城创想科技有限责任公司" target="_blank">Tianjin Netcity Networking Inc.</a> All Rights Reserved.</h6>
...
function step3(&$install_error,&$install_recover){
	...
	if (!$db_host || !$db_port || !$db_user || !$db_pwd || !$db_nae || !$db_prefix || !$admin || !$password){
	...
	//$sql = file_get_contents("../../cms/install/{$_charset}_add.sql");
	//if ($_POST['demo_data'] == '1'){
	//	$sql .= "\r\n".file_get_contents("data/{$_charset}_add.sql");
	//}
	//$sql = str_replace("\r\n", "\n", $sql);
	//runquery($sql,$db_prefix,$db);
	...
	//管理员帐号密码
	$db->query("INSERT INTO {$db_prefix}admin (`admin_id`,`admin_name`,`admin_password`) VALUES ('1','$username','". md5($password) ."');");
	//新增一个标识文件，用来屏蔽重新安装
	$fp = @fopen('lock','wb+');
	@fclose($fp);
...
function write_config($url) {
	...
	$configfile = str_replace("===app_key1===",	md5(random(10)), $configfile);
	$configfile = str_replace("===app_key2===",	md5(random(10)), $configfile);
	$configfile = str_replace("===app_key3===",	md5(random(10)), $configfile);
	$configfile = str_replace("===app_key4===",	md5(random(10)), $configfile);
